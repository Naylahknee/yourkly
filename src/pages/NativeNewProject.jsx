...
export default function NativeNewProject(){
 ...
 return <div className="welcome-page onboarding-page mobile-flow mobile-flow--native"><div className="onboarding-shell">
  ...
  {step===1?<><h1>Make your project</h1><p className="onboarding-lead">Start with what you call it and what you're making.</p>
  ...
  </>:<><button className="back-link" onClick={()=>setStep(1)}>← Back</button><h1>Set it up</h1><p className="onboarding-lead">Yourkly picked safe defaults. Change only what matters to you.</p>
   <div className="newproject-card config-card"><div><div className="newproject-label">What kind of project is this?</div><div className="config-grid">{PROJECT_TYPES.map(t=><button key={t.id} type="button" className={`ai-tool${projectType===t.id?' ai-tool--on':''}`} onClick={()=>setProjectType(t.id)}><strong>{t.label}</strong><small>{t.description}</small></button>)}</div></div>
   <div><div className="newproject-label">Who can see this project?</div><div className="newproject-choices"><button type="button" className={`ai-tool${visibility==='private'?' ai-tool--on':''}`} onClick={()=>setVisibility('private')}>Only me / people I invite</button><button type="button" className={`ai-tool${visibility==='public'?' ai-tool--on':''}`} onClick={()=>setVisibility('public')}>Anyone</button></div></div>
   ...
   <div><div className="newproject-label">How can other people use this project?</div><select className="newproject-input" value={usage} onChange={e=>setUsage(e.target.value)}><option value="private">Don't give reuse permission</option><option value="mit">Allow reuse with credit (MIT)</option></select></div>
